...
class LanguageSetupManager {
    ...
    static var language: LanguageType {
        get {
            return LanguageSetupManager.LanguageType(rawValue: UserDefaults.standard.string(forKey: "LanguageType") ?? "en") ?? .en
        }
        set {
...
//        }
    ...
    static func localizedString(key: String, comment: String? = nil) -> String {
        var translation = LanguageSetupManager.getTranslationValue(forKey: key)
...
extension Bundle {
    @objc func specialLocalizedString(key: String, value: String?, table tableName: String?) -> String {
        let currentLanguage = LanguageSetupManager.isRTL ? "ar" : "en"
        var bundle = Bundle();
...
// MARK: - HELPER METHODS
extension LanguageSetupManager {
